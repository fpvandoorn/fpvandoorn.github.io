...
\newcommand{\tactic}{\textit{tac}\xspace} % also used for tactic sequences
...
\begin{longtable}{@{}lp{113mm}@{}}
  ...
  \lean{have h :} \proposition\ \lean{:= by} & add hypothesis \lean{h} after proving it using tactics. \warning \textbf{Do not use for data!} \\
  ...
  \lean{simp\_rw [}\textit{expr1}\lean{, }\ldots\lean{]} & like \lean{rw}, but uses \lean{simp only} at each step. \\
  ...
  \lean{decide} & run a decision procedure. \\
  ...
  \lean{pick\_goal} \nat & move goal \nat to the front. \\
  \lean{all\_goals} \tactic & apply \tactic to all goals. \\
  \lean{try} \tactic & apply \tactic only if it succeeds. \\
...
This is a quick overview of the most common tactics in Lean with only a short description. To learn more about a tactic or learn its precise syntax or variants, consult its docstring. You can use \lean{\#help tactic} \tactic\ to find the full syntax, variants and more information about \tactic. Use \lean{\#help tactic} to list all imported tactics.
\bigskip
